...
const LineChartExpenseTimeline = props => {
  ...
  if (expenses && currentUser && selectedMonth && selectedYear && settings) {
    ...
    let allDatesInSelectedMonth = utils.getAllTheDatesInAMonth(
      selectedYear,
      selectedMonth
    );
    let TotalInThatDay = allDatesInSelectedMonth.map(date => {
      var expensesOnThatDate = usersExpensesInSelectedMonthAndYear.filter(exp =>
        Number(exp.value.date === date)
      );
      totals[date] =
    ...
    return (
      <div>
        ...
        <div className="col-sm-12 mobileNoPadding" style={lineArea}>
          <Line
            data={data}
            options={window.screen.width > 720 ? optionsDesktop : options}
