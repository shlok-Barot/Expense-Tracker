...
export const expensesinMonthAndYear = (
  eachExpense,
  currentUser,
  selectedMonth,
  selectedYear
  ...
  return eachExpense
    ...
    .filter(
      (elem) =>
        new Date(elem.value.date).getFullYear().toString() ===
        selectedYear.toString()
    )
    .filter(
      (elem) =>
        new Date(elem.value.date).getMonth().toString() === selectedMonth
    );
};

// expenses in selected year
export const expensesinSelectedYear = (
  eachExpense,
  currentUser,
  selectedYear
) => {
...
export const expensesInDate = (eachExpense, currentUser, date) => {
  return eachExpense.filter(
    (elem) => elem.value.uid === currentUser.uid && elem.value.date === date
  );
};
...
export const expensesinMonth = (
  ...
  return eachExpense
    ...
    .filter(
      (elem) => new Date(elem.value.date).getMonth().toString() === MonthNumber
    );
};
...
export const expensesToday = (eachExpense, currentUser) => {
  return eachExpense
    ...
    .filter(
      (elem) => new Date(elem.value.date).getDate() === new Date().getDate()
    );
};
...
export const expensesThisWeek = (eachExpense, currentUser) => {
  return eachExpense.filter(
    (elem) =>
      elem.value.uid === currentUser.uid &&
      moment(elem.value.date, "MM/DD/YYYY").week() ===
        moment(moment(new Date()), "MM/DD/YYYY").week() &&
      moment(elem.value.date, "MM/DD/YYYY").year() ===
...
export const totalExpensesInEachMonthOfThisYear = (
  expenses,
  eachExpense,
  currentUser,
  selectedYear
  ...
  for (var i = 0; i <= 11; i++) {
    expensesOfAllMonthsInThisYear.push(
      totalExpense(
        expensesinMonth(eachExpense, currentUser, String(i), selectedYear)
      )
    );
  }
...
export const calculateTotalForAllCategories = (expenses) => {
  ...
  const totalForACategory = function (expenses, category) {
    let temp = expenses
      .filter((elem) => elem.value.category === category)
...
export const filterExpensesByCriteria = (
  startDate,
  endDate,
  category,
  expenseFrom,
  ...
  filteredExpenses = filteredExpenses
    ...
    .filter((elem) => {
      return (
        Number(elem.value.expense) >= Number(expenseFrom) &&
...
export const loanTakenOrGivenAmt = (thisUsersLoans, takenOrGiven) => {
  if (thisUsersLoans.length && takenOrGiven) {
    let loans = thisUsersLoans.filter(
      (elem) =>
        elem.value.loanType === takenOrGiven && elem.value.status === "Pending"
    );

    return loans.length
      ? loans
          .map((elem) => Number(elem.value.amount))
          .reduce((prev, cur) => prev + cur)
      : 0;
  } else {
    return 0;
