...
class LoanRow extends Component {
  ...
  handleClick(e) {
    ...
    if (window.confirm(message)) {
      firebase.db
        .ref(`loanTable/${this.props.user.uid}/${this.props.loanId}`)
        .remove();
    }
  }
  ...
  render() {
    const conditionForDay =
      this.props.loan.value.day || moment(this.props.loan.value.date).day();
    ...
    return (
      <tr
        key={this.props.loanId}
        id={this.props.loanId}
        style={loanStatusStyle}
      >
        <td data-th="No">
          {this.props.num + 1}
