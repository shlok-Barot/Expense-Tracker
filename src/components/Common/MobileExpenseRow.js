...
class MobileExpenseRow extends Component {
  ...
  handleClick(e) {
    ...
    if (window.confirm(message)) {
      firebase.db
        .ref(`expenseTable/${this.props.user.uid}/${this.props.expenseId}`)
        .remove();
    }
  }
  ...
  render() {
    ...
    let catName = this.props.settings.editedCategories[
      this.props.expense.value.category
    ]
      ? this.props.settings.editedCategories[this.props.expense.value.category]
      : this.props.expense.value.category;

    const nightModeTitle = {
      background: this.props.settings
        ? this.props.settings.mode === "night" ? "#2C3034" : "#fff"
        : "#fff",
      color: this.props.settings
        ? this.props.settings.mode === "night" ? "#BBBBBB" : "#212529"
        : "#212529",
    ...
    return (
      <div className="option">
        ...
        <input
          type="checkbox"
          id={`toggle${this.props.expenseId}`}
          className="toggle"
        />
        <label
          className="title"
          style={nightModeTitle}
          for={`toggle${this.props.expenseId}`}
        >
          ...
          <span className="mobile-row-header">
            <i
              className={`fa fa-${utils.categoryIcon(
                this.props.expense.value.category
              )}`}
              style={lessFont}
              aria-hidden="true"
            ...
            <span className="truncate">
              {this.props.expense.value.comments}
            </span>
          </span>
          <span className="mobile-row-header last-item">
            <i
              className={`fa ${utils.setCurrencyIcon(
